...
/*   Updated: 2018/06/29 20:05:03 by bhamidi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nm.h"

const char	*g_sect_tab[3] = {
	SECT_TEXT, SECT_DATA, SECT_BSS
};

const char	g_sect_char_tab[3] = {
	'T', 'D', 'B'
};
...
char	get_str_sign(const char *sectname, const char *segname)
{
	...
	if (ft_strcmp(segname, SEG_TEXT) && ft_strcmp(segname, SEG_DATA))
		return ('S');
	while (++i < 3)
		if (!ft_strcmp(g_sect_tab[i], sectname))
			return (g_sect_char_tab[i]);
	return ('S');
}
...
char	get_segment(t_info *inf, uint8_t ordinal, int mode)
{
	...
	uint32_t			acc;
	char				c;
	const unsigned int	value = mode ? LC_SEGMENT_64 : LC_SEGMENT;
	...
	while (++i < inf->ncmds)
	{
		...
		if (c)
			return (c);
		if (check(inf, (void*)lc + rev(lc->cmdsize, 0, sizeof(uint32_t),
						inf->endian), sizeof(*lc)))
			return (0);
